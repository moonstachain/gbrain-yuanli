...
type Manifest = {
  ...
  comparison: { audited_main_commit: string; relationship: string; audited_main_admission?: string; blocker_ids?: string[] };
...
if (!candidate || candidate === manifest.production.commit) {
  console.log(`runtime replay admission: PASS frozen-production (${manifest.production.version} @ ${manifest.production.commit.slice(0, 12)})`);
  process.exit(0);
}
if (candidate === manifest.comparison.audited_main_commit) {
  const blockers = manifest.comparison.blocker_ids ?? [];
  if (manifest.comparison.audited_main_admission === 'blocked' || blockers.length > 0) {
    console.error(`runtime replay admission: BLOCKED frozen-audited-main candidate=${candidate}`);
    for (const id of blockers) console.error(`- missing ${id}`);
    process.exit(3);
  }
  console.log(`runtime replay admission: PASS frozen-audited-main candidate=${candidate}`);
  process.exit(0);
}
...
if (git('cat-file', '-e', `${manifest.production.commit}^{commit}`).exitCode !== 0) {
  console.error(`runtime replay admission: BLOCKED replay_history_incomplete production=${manifest.production.commit}`);
  process.exit(2);
}
